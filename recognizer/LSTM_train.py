...
from recognizer.LSTM_model import *
...
def run(config, datadicts=None):
    ...
    hmm = HMM()
    f_dim = 3 * config["num_ceps"]
    c_dim = config["left_context"] + config["right_context"] + 1
    idim = f_dim * c_dim
    odim = hmm.get_num_states()

    model = LSTM_Model(idim=idim, odim=odim, hidden_dim=config["hidden_size"])
    ...
    for epoch in range(epochs):
        ...
        if evalscore <= evalacc_best:
            ...
            OUTPUT_DIR = os.path.join(modeldir,'bestaccmodel_lstm.pkl') # save trained model

...
# Append a specified number of preceding and succeeding frame features (left context and right context) to the features of each frame
def add_context(feats, left_context=1, right_context=2):
    c_dim = left_context + 1 + right_context
    new_feats = np.zeros((feats.shape[0], feats.shape[1], c_dim))

    # Iterate from -left_context to +right_context, e.g. for left=1 & right=2, offsets=[-1, 0, 1, 2]
    offsets = np.arange(-left_context, right_context + 1)

    for i, offset in enumerate(offsets):
        # np.roll: positive shifts move data "down" (bringing past frames to current index)
        rolled = np.roll(feats, -offset, axis=0)

        # Pad with first/last frame at the boundaries
        if offset < 0:
            # Pad with first frame
            rolled[:abs(offset)] = feats[0]
        elif offset > 0:
            # Pad with last frame
            rolled[-offset:] = feats[-1]
...
# Extract features and append a corresponding context
def compute_features_with_context(audio_file, window_size=25e-3, hop_size=10e-3,
                                  feature_type="STFT", n_filters=24, fbank_fmin=0, fbank_fmax=8000, num_ceps=13,
                                  left_context=4, right_context=4):
    # Extract features
    feats = compute_features(audio_file, window_size=window_size, hop_size=hop_size,
                     feature_type=feature_type, n_filters=n_filters, fbank_fmin=fbank_fmin, fbank_fmax=fbank_fmax, num_ceps=num_ceps)
    
    # Add context to features
    feats_with_context = add_context(feats, left_context=left_context, right_context=right_context)
    return feats_with_context
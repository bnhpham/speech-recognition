...
from gender_classification.local.model import *
from gender_classification.local.utils import *
...
def train(dataset, model, device, optimizer=None, criterion=None):
    ...
    for X, y, _ in tqdm(dataset, desc="Training the model", unit="batch"):
        ...
        # compute gradient from the loss value through back-propagation algorithm
        loss.backward()

        # update model parameters
        optimizer.step()

        # compute predictions from the posterior probabilities
        # 0 = male, 1 = female
        # If the probability is smaller than 0.5, we classify the voice as male. Otherwise, it's female.
        preds = torch.round(outputs)

        # compute correctly identified samples
...
def evaluation(dataset, model, device):
    ...
    # prepare a dictionary to save the predicted results
    outputdict = {}
    ...
    # traverse each batch of samples
    with torch.no_grad():
        for X, y, file_name in tqdm(dataset, desc="Evaluating the model", unit="batch"):
            # move data onto gpu if gpu available
            X, y = X.to(device), y.to(device)
            ...
            # convert posterior probabilities from tensor to list
            probs = outputs.tolist()
            
            # save predicted results in dictionary
            for i in range (len(X)):
                outputdict[file_name[i]] = [preds[i].item(), y[i].item(), probs[i]]
